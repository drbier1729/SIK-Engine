...
namespace Collision {
	...
	Bool BVHierarchy::MoveBoundingVolume(BVHandle handle, AABB const& aabb, Vec3 const& displacement)
	{
		...
		{
			SIK_ASSERT(tree[index].IsLeaf(), "Cannot move internal nodes");

			Vec3 const d = BV::displacementMultiplier * displacement;

			AABB predictedAABB = aabb.Expanded(BV::enlargeFactor);
			Vec3 predictedMin = predictedAABB.Min();
			Vec3 predictedMax = predictedAABB.Max();
			if (d.x < 0.0f) { predictedMin.x += d.x; }
			else { predictedMax.x += d.x; }
			if (d.y < 0.0f) { predictedMin.y += d.y; }
			else { predictedMax.y += d.y; }
			if (d.z < 0.0f) { predictedMin.z += d.z; }
			else { predictedMax.z += d.z; }
			predictedAABB.SetMinMax(predictedMin, predictedMax);
	...
	{
		SIK_ASSERT(0 <= index && index < capacity, "Index out of range");
		SIK_ASSERT(not nodes[index].IsFree(), "Node is dead");
		return nodes[index];
	}
	...
	Int32 BVHierarchy::NodePool::Create(AABB const& aabb, void* userData)
	{
		...
		{
			SIK_ASSERT(size == capacity, "Growing unecessarily");
			capacity = Reserve(capacity * 2);
		}

		Int32 const index = firstFree;
		SIK_ASSERT(0 <= index && index < capacity, "Bad index");
	...
	{
		SIK_ASSERT(0 <= nodeIndex && nodeIndex < capacity, "Bad index");
		SIK_ASSERT(size > 0, "Empty array");
	...
	Int32 BVHierarchy::NodePool::Reserve(Int32 newCapacity)
	{
		...
		{
			//SIK_ASSERT(0 <= capacity && capacity < newCapacity, "Reserving unecessarily");
			//SIK_ASSERT(0 <= capacity && capacity < maxCapacity, "Max capacity reached");
			return capacity;
		}
		...
		SIK_ASSERT(size < capacity&& capacity > 0, "Something went wrong");
	...
	{
		SIK_ASSERT(aIdx < tree.Capacity(), "Index invalid");
		...
		if (balance > 1)
		{
			...
			if (c.parent != NullIdx)
			{
				...
				{
					SIK_ASSERT(cParent.children[1] == a.index, "Parent is incorrect");
		...
		else if (balance < -1)
		{
			...
			if (b.parent != NullIdx)
			{
				...
				{
					SIK_ASSERT(bParent.children[1] == a.index, "Parent incorrect");
	...
	Int32 BVHierarchy::FindBestSiblingFor(Int32 newNodeIndex)
	{
		...
		while (not tree[idx].IsLeaf() && idx != prevIdx)
		{
			...
			for (Int8 i = 0; i < 2; ++i)
			{
				SIK_ASSERT(curr.children[i] != NullIdx, "Internal nodes must have 2 valid children");
	...
	{
		SIK_ASSERT(0 <= index && index < tree.Capacity(), "Bad index");
		while (index != NullIdx)
		{
			index = Balance(index);

			BVHNode& curr = tree[index];
			SIK_ASSERT(0 <= curr.children[0] && curr.children[0] < tree.Capacity() &&
